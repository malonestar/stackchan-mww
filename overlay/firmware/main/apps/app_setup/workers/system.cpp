...
/* -------------------------------- Wake Word ------------------------------- */
// microWakeWord model picker. the labels and keys here mirror the embedded model
// list in micro_wake_word.cc (kModels[]) — keep them in sync if you add or remove
// a model.
struct WakeModelOption_t {
    std::string name;  // user-facing label
    std::string key;   // NVS value stored under mww/model
};

static const std::vector<WakeModelOption_t> _wake_model_list = {
    {"Hey Stack Chan", "stackchan"},
    {"Hey Frank", "frank"},
    {"Hey M5", "m5"},
};

WakeWordSetupWorker::WakeWordSetupWorker()
{
    _panel = std::make_unique<uitk::lvgl_cpp::Container>(lv_screen_active());
    _panel->setPadding(0, 0, 0, 0);
    _panel->setBgColor(lv_color_hex(0xEDF4FF));
    ...
    _label = std::make_unique<uitk::lvgl_cpp::Label>(_panel->get());
    _label->setText("Wake Word");
    _label->setTextFont(&lv_font_montserrat_24);
    _label->setTextColor(lv_color_hex(0x26206A));
    _label->align(LV_ALIGN_CENTER, 0, -70);

    // Model list (newline-separated for the dropdown)
    std::string options;
    for (const auto& m : _wake_model_list) {
        options += m.name + "\n";
    }
    if (!options.empty()) {
        options.pop_back();  // drop trailing newline
    }

    // Raw lv_dropdown — no uitk wrapper exists. Child of _panel, so LVGL frees it
    // when _panel is destroyed; no manual delete needed.
    _dropdown = lv_dropdown_create(_panel->get());
    lv_dropdown_set_options(_dropdown, options.c_str());
    lv_obj_set_width(_dropdown, 240);
    lv_obj_align(_dropdown, LV_ALIGN_CENTER, 0, -10);
    lv_obj_set_style_text_font(_dropdown, &lv_font_montserrat_16, LV_PART_MAIN);
    lv_obj_set_style_text_color(_dropdown, lv_color_hex(0x26206A), LV_PART_MAIN);
    lv_obj_set_style_bg_color(_dropdown, lv_color_hex(0xB8D3FD), LV_PART_MAIN);
    lv_obj_set_style_border_width(_dropdown, 0, LV_PART_MAIN);
    lv_obj_set_style_radius(_dropdown, 12, LV_PART_MAIN);
    lv_obj_t* list = lv_dropdown_get_list(_dropdown);
    if (list) {
        lv_obj_set_style_text_font(list, &lv_font_montserrat_16, LV_PART_MAIN);
        lv_obj_set_style_bg_color(list, lv_color_hex(0xB8D3FD), LV_PART_MAIN);
        lv_obj_set_style_text_color(list, lv_color_hex(0x26206A), LV_PART_MAIN);
        lv_obj_set_style_bg_color(list, lv_color_hex(0x615B9E), LV_PART_SELECTED);
    }

    // Preselect the currently active model
    std::string current_model = GetHAL().getWakeModel();
    for (size_t i = 0; i < _wake_model_list.size(); ++i) {
        if (current_model == _wake_model_list[i].key) {
            lv_dropdown_set_selected(_dropdown, i);
            break;
        }
    }

    _btn_save = std::make_unique<uitk::lvgl_cpp::Button>(_panel->get());
    _btn_save->label().setText("Save");
    _btn_save->label().setTextFont(&lv_font_montserrat_16);
    _btn_save->setSize(130, 50);
    _btn_save->align(LV_ALIGN_CENTER, -68, 70);
    _btn_save->onClick().connect([&]() { _save_flag = true; });
    _btn_save->setRadius(12);
    _btn_save->setShadowWidth(0);
    _btn_save->setBgColor(lv_color_hex(0x615B9E));

    _btn_cancel = std::make_unique<uitk::lvgl_cpp::Button>(_panel->get());
    _btn_cancel->label().setText("Cancel");
    _btn_cancel->label().setTextFont(&lv_font_montserrat_16);
    _btn_cancel->setSize(130, 50);
    _btn_cancel->align(LV_ALIGN_CENTER, 68, 70);
    _btn_cancel->onClick().connect([&]() { _cancel_flag = true; });
    _btn_cancel->setRadius(12);
    _btn_cancel->setShadowWidth(0);
    _btn_cancel->setBgColor(lv_color_hex(0xB8D3FD));
    _btn_cancel->label().setTextColor(lv_color_hex(0x26206A));

    _note = std::make_unique<uitk::lvgl_cpp::Label>(_panel->get());
    _note->setText("Saving reboots the device");
    _note->setTextFont(&lv_font_montserrat_14);
    _note->setTextColor(lv_color_hex(0x7A78A0));
    _note->align(LV_ALIGN_CENTER, 0, 104);
}
...
void WakeWordSetupWorker::update()
{
    if (_cancel_flag) {
        _cancel_flag = false;
        _is_done     = true;  // leave the page, no change
        return;
    }
    ...
    uint32_t selected_id = lv_dropdown_get_selected(_dropdown);
    if (selected_id >= _wake_model_list.size()) {
        return;
    }

    const auto& selected = _wake_model_list[selected_id];
    GetHAL().setWakeModel(selected.key);
    mclog::tagInfo(_tag, "wake model set to: {} ({}), rebooting to apply", selected.name, selected.key);

    // Reboot-to-switch: MicroWakeWord reads mww/model at construction, so the new
    // model loads on the next boot. Plain reboot() -> boots to the launcher home
    // (do NOT auto-enter the agent).
    GetHAL().reboot();
}
