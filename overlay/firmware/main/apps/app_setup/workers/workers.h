...
/**
 * @brief microWakeWord model picker. Writes the active model id to NVS
 *        (mww/model via Hal::setWakeModel) and reboots so MicroWakeWord reloads
 *        it at construction (reboot-to-switch). Boots to the launcher home.
 */
class WakeWordSetupWorker : public WorkerBase {
public:
    WakeWordSetupWorker();
    ~WakeWordSetupWorker();
    void update() override;
...
private:
    std::unique_ptr<uitk::lvgl_cpp::Container> _panel;
    std::unique_ptr<uitk::lvgl_cpp::Label> _label;
    std::unique_ptr<uitk::lvgl_cpp::Label> _note;
    std::unique_ptr<uitk::lvgl_cpp::Button> _btn_save;
    std::unique_ptr<uitk::lvgl_cpp::Button> _btn_cancel;
    lv_obj_t* _dropdown = nullptr;  // raw lv_dropdown (no uitk wrapper); freed with _panel
