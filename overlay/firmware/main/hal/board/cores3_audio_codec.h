...
private:
    ...
    // One-shot guard: the first esp_codec_dev_open after boot intermittently brings
    // the ES7210 up sub-real-time ("dead" audio). Prime once with a throwaway
    // open/close so the operative open is always a "second open after close". See EnableInput.
    bool input_primed_ = false;
