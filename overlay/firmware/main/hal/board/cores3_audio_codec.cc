...
#include <esp_private/periph_ctrl.h>    // I2S0 peripheral reset (see CreateDuplexChannels)
#include <soc/periph_defs.h>            // PERIPH_I2S0_MODULE
#include <esp_rom_sys.h>                // esp_rom_delay_us (codec-dev prime cycle)
...
void CoreS3AudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
    ...
    // esp_restart() (web-UI reboot / reboot-to-switch) does NOT reset the I2S0
    // peripheral the way an EN-pin/power-on reset does; after a soft reboot the
    // RX path intermittently came up delivering sub-real-time audio (mWW dead,
    // pipeline ~40% of real-time), cleared only by a full hardware reset. Force
    // the module to a known power-on state before (re)creating channels so a
    // soft reboot behaves like a clean boot for audio.
...
void CoreS3AudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        // Prime cycle (first enable after boot only). The very first codec-dev open
        // intermittently brings the ES7210 up sub-real-time -> mWW sees ~0 ("dead"
        // audio), otherwise cleared only by leaving/re-entering the agent -- which is
        // itself a close+reopen at this layer. Replicate that recovery deterministically:
        // a throwaway open+close so the operative open below is always a "second open
        // after close", the state that locks cleanly. Best-effort (no abort) so a
        // priming hiccup never blocks boot. This is the layer the I2S0 reset/re-latch missed.
        if (!input_primed_) {
            ESP_ERROR_CHECK_WITHOUT_ABORT(esp_codec_dev_open(input_dev_, &fs));
