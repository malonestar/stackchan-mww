...
public:
    ...
    /* -------------------------------- Wake Word ------------------------------- */
    // Active microWakeWord model id ("stackchan" | "frank" | "m5"), persisted in
    // NVS (namespace "mww", key "model"). MicroWakeWord reads it at construction,
    // so a change takes effect on the next boot (reboot-to-switch).
    std::string getWakeModel();
    void setWakeModel(const std::string& v);
